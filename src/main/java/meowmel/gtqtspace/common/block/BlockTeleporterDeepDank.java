...
public class BlockTeleporterDeepDank extends Block implements ITileEntityProvider {
    ...
    public BlockTeleporterDeepDank() {
        ...
        setHardness(20.0F);
        setRegistryName("teleporter_spacestation");
        setTranslationKey("teleporter_spacestation");
    ...
    {
        return worldIn.provider.getDimension() == 50 ? -1.0F : super.getBlockHardness(blockState, worldIn, pos);
    }
        ...
        if(worldIn.isRemote)
            if(xdiff <= 2 && xdiff >= -2 && zdiff <= 2 && zdiff >= -2) {
                if(playerIn.posY >= pos.getY() && playerIn.posY <= pos.getY()+1)
                    if(playerIn.dimension == 0 || playerIn.dimension == 50)
                        PacketDispatcher.sendToServer(new TeleportMessage(pos));
                    else playerIn.sendStatusMessage(new TextComponentString("SpaceStation teleporters don't work outside the Overworld!"), true);
