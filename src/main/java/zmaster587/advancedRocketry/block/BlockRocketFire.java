    ...
    private void tryCatchFire(World worldIn, BlockPos pos, int chance, Random random, int age, EnumFacing face) {
        ...
        if (random.nextInt(chance) < i) {
            ...
            if (iblockstate.getBlock() == Blocks.TNT) {
                worldIn.setBlockState(pos, Blocks.TNT.getDefaultState().withProperty(BlockTNT.EXPLODE, true), 3);
                worldIn.createExplosion(null, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, 4.0F, true);
            }
        }
    }
