...
public class ItemProjector extends Item implements IModularInventory, IButtonInventory, INetworkItem {
	...
	private void clearStructure(World world, TileMultiBlock tile, @Nonnull ItemStack stack) {
		...
		EnumFacing direction = EnumFacing.byIndex(getDirection(stack));
	...
	public EnumActionResult onItemUseFirst(EntityPlayer player, World world,
		...
		if(!player.isSneaking() && id != -1 && world.isRemote) {
			EnumFacing dir = EnumFacing.byIndex(ZUtils.getDirectionFacing(player.rotationYaw - 180));
	...
	public void useNetworkData(EntityPlayer player, Side side, byte id,
		...
		if(id == 0) {
			...
			RebuildStructure(player.world, this.machineList.get(getMachineId(stack)), stack, vec.x, vec.y, vec.z, EnumFacing.byIndex(getDirection(stack)));
		}
		else if(id == 2) {
			...
			if(getMachineId(stack) != -1)
				RebuildStructure(player.world, this.machineList.get(getMachineId(stack)), stack, x, y, z, EnumFacing.byIndex(dir));
		}
	}
}