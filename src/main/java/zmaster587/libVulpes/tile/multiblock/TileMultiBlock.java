		...
		for(int y = 0; y < structure.length; y++) {
			for(int z = 0; z < structure[0].length; z++) {
				for(int x = 0; x< structure[0][0].length; x++) {
					...
					int chunkX = globalPos.getX() >> 4; // 等价于 BlockPos.getX() / 16
					int chunkZ = globalPos.getZ() >> 4; // 等价于 BlockPos.getZ() / 16
		...
		for(int y = 0; y < structure.length; y++) {
			for(int z = 0; z < structure[0].length; z++) {
				for(int x = 0; x< structure[0][0].length; x++) {
					...
					int globalX = pos.getX() + (x - offset.x)*front.getZOffset() - (z-offset.z)*front.getXOffset();
					int globalY = pos.getY() - y + offset.y;
					int globalZ = pos.getZ() - (x - offset.x)*front.getXOffset()  - (z-offset.z)*front.getYOffset();
					BlockPos globalPos = new BlockPos(globalX, globalY, globalZ);
