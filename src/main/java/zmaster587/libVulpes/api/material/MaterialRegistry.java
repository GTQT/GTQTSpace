...
public class MaterialRegistry {
	...
	public void registerOres(CreativeTabs tab) {
		...
		for(int i = 0; i < numberOfOreBlocks; i++) {
			...
			metalBlocks.setRegistryName(metalBlockName).setCreativeTab(tab).setHardness(4f);
			...
			ores.setRegistryName(name).setCreativeTab(tab).setHardness(4f);
			...
			coilBlocks.setRegistryName(coilName).setCreativeTab(tab).setHardness(4f);
			coilBlocks.numBlocks = (byte)Math.min(len - (16*i), 16);
			coilBlocks.product = AllowedProducts.getProductByName("COIL");

			ores.setTranslationKey(name + i);
			metalBlocks.setTranslationKey(metalBlockName + i);
			coilBlocks.setTranslationKey(coilName + i);
